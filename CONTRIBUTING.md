...
go vet ./...
golangci-lint run     # config in .golangci.yml; mostly defaults
```

CI runs all of these on every push and PR, plus `govulncheck` and a `docker build` of the release image.
