...
- Until the agent is deployed, Alertmanager logs failed notification attempts — expected and harmless

## Monitoring stack

Compose project at `/mnt/user/appdata/monitoring/` on Tower (`ssh tower` → root@tower.local); manage with `docker compose` from that directory. Configs: `alertmanager.yml`, `alerts.yml`, `prometheus.yml` (pre-change backups as `.bak`).
...
`InstanceDown`, `HostHighMemory`, `HostHighCPU`, `ContainerRestarting`. `ContainerRestarting` sets a `container` label with the Docker container name — the primary key for Target resolution.
